...
// ************************ GET ALL PRODUCTS **********************************************
export function getProductosOffertZalandoAPIAction(list_brand_sizes) {
  return async (dispatch) => {
    dispatch(getProductosOffertZalandoAPI(true));

    try {
      console.log("-getProductosOffertZalandoAPIAction-")

      let response = await axios({
        method: 'POST',
        url: urlProductos + 'byBrandJson',
        data: list_brand_sizes
      });

      console.log("-response.data-")
      console.log(response.data)

      dispatch(getProductosOffertZalandoAPIExito(response.data));
    } catch (error) {
      console.log(error.response);
      dispatch(getProductosOffertZalandoAPIError(true));
    }
  };
}

const getProductosOffertZalandoAPI = (estado) => ({
  type: GET_PRODUCT_OFFERT_ZALANDO,
  payload: true,
});

const getProductosOffertZalandoAPIExito = (categorias) => ({
  type: GET_PRODUCT_OFFERT_ZALANDO_EXITO,
  payload: categorias,
});

const getProductosOffertZalandoAPIError = (estado) => ({
  type: GET_PRODUCT_OFFERT_ZALANDO_ERROR,
  payload: estado,
});
